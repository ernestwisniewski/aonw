...
const MIN_AUTHORED_COLS: u16 = 5;
const MIN_AUTHORED_ROWS: u16 = 5;
...
impl MapDocument {
    ...
    pub fn try_new(map: MapDefinition, default_zoom: f64) -> Result<Self, MapValidationError> {
        if map.cols() < MIN_AUTHORED_COLS {
            return Err(MapValidationError::new(
                "$.cols",
                format!("must be at least {MIN_AUTHORED_COLS} for an authored map document"),
            ));
        }
        if map.rows() < MIN_AUTHORED_ROWS {
            return Err(MapValidationError::new(
                "$.rows",
                format!("must be at least {MIN_AUTHORED_ROWS} for an authored map document"),
            ));
        }
