...
use aonw_contracts::{MapObjectiveTypeDto, ReplayEventDto};
use aonw_engine::{DominationThresholdReachedEvent, MapObjectiveSecuredEvent};

pub(super) fn map_secured(value: &MapObjectiveSecuredEvent) -> ReplayEventDto {
    ReplayEventDto::MapObjectiveSecured {
...
pub(super) fn domination(value: &DominationThresholdReachedEvent) -> ReplayEventDto {
    ReplayEventDto::DominationThresholdReached {
        player_id: value.player_id().as_str().to_owned(),
