...
class _YieldDeltaRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final beforeFactor = _factor(item.before, maxValue);
    final afterFactor = _factor(item.after, maxValue);
...
double _factor(int value, int maxValue) {
  if (maxValue <= 0) return 0;
  return (value.abs() / maxValue).clamp(0.0, 1.0);
}

class _YieldDeltaHeader extends StatelessWidget {
  const _YieldDeltaHeader({required this.item});
