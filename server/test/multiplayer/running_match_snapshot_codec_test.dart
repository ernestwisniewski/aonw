...
void main() {
  ...
  group('DecodedRunningMatchSnapshot', () {
    ...
    test(
      'withState creates a fresh canonical view without mutating source',
      () {
        final fixture = _fixture();
        final decoded = codec.decode(
          match: fixture.match,
        ...
        final sourceCanonical = decoded.canonical;
        ...
        final refreshed = decoded.withState(replacement);

        expect(refreshed.state, same(replacement));
        expect(refreshed.canonical, isNot(same(sourceCanonical)));
        expect(refreshed.canonical.session.submittedPlayerIds, {'player-1'});
        expect(decoded.state.runtimeState.submittedPlayerIds, isEmpty);
        expect(decoded.canonical, same(sourceCanonical));
        expect(decoded.wire, same(fixture.wire));
        expect(
          (decoded.wire.state['runtimeState']! as Map).containsKey(
            'submittedPlayerIds',
