...
void main() {
  group('unit action persistent/domain adapter parity', () {
    ...
    test('skip has exact state and interaction parity', () {
      final states = _states(
        units: [_unit(movementPoints: 3, queuedPath: _queuedPath())],
        artifacts: const [_artifact],
      ...
      final persistent = const PersistentUnitActionResolver().skipUnitTurn(
        state: states.persistent,
        command: command,
        actorPlayerId: _playerId,
      );
      final domain = const DomainUnitActionCommandResolver().skipUnitTurn(
        state: states.domain,
        interaction: states.interaction,
        command: command,
        actorPlayerId: _playerId,
      ...
      _expectAcceptedParity(states, persistent, domain);
      expect(persistent.state.units.single.movementPoints, 0);
      expect(persistent.state.units.single.queuedPath, isNull);
      expect(domain.interaction.cityFoundingDraft, isNull);
      expect(
    ...
    test('skip reject preserves both state boundary identities', () {
      final states = _states(units: [_unit()]);
      const command = SkipUnitTurnCommand('missing');
    ...
    test('fortify clears interaction owned by the unit with exact parity', () {
      final states = _states(
        units: [_unit(movementPoints: 2, queuedPath: _queuedPath())],
        artifacts: const [_artifact],
        interaction: PersistedInteractionState(
          cityFoundingDraft: _draft('unit_1'),
      ...
      const command = FortifyUnitCommand('unit_1');

      final persistent = const PersistentUnitActionResolver().fortifyUnit(
        state: states.persistent,
        command: command,
        actorPlayerId: _playerId,
      );
      final domain = const DomainUnitActionCommandResolver().fortifyUnit(
        state: states.domain,
        interaction: states.interaction,
        command: command,
        actorPlayerId: _playerId,
      ...
      expect(persistent.state.units.single.movementPoints, 0);
      expect(persistent.state.units.single.queuedPath, isNull);
      expect(persistent.state.units.single.posture, UnitPosture.fortified);
      expect(domain.interaction, PersistedInteractionState.empty);
    });

    test('fortify preserves unrelated interaction with exact parity', () {
      final interaction = PersistedInteractionState(
      ...
      expect(domain.interaction.pendingAction, interaction.pendingAction);
    });

    test('fortify busy reject preserves both state boundary identities', () {
      final states = _states(
        units: [
          _unit(
            workerJob: const WorkerJob(
      ...
      _expectRejectedIdentity(states, persistent, domain, reason: 'unit_busy');
    });
  });
}
