...
const DEFAULT_MAP := "res://../../assets/maps/myranth/map.json"
...
func _open(source_path: String) -> void:
	...
	var result := _open_map.execute(source_path)
	if not result["ok"]:
		_status.text = "Błąd: %s" % result["message"]
		return

	_current_document = result["document"]
	_surface.present(_current_document, result["texture"])
	...
	if not missing.is_empty():
		_status.text += " · brak tekstur: %d" % missing.size()

func _on_map_presented(world_size: Vector2) -> void:
	_camera_rig.frame_map(world_size, _current_document.default_zoom())
